...
def chart_weather_distribution(df: pd.DataFrame) -> alt.Chart:
    month_order = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
    return (
        alt.Chart(df)
        .mark_bar()
        .encode(
            x=alt.X("month_name:N", title="Month", sort=month_order),
            y=alt.Y("count():Q", stack="normalize", title="Proportion of Days"),
...
def chart_temp_range_interactive(df: pd.DataFrame) -> alt.Chart:
    legend_select = alt.selection_point(fields=["weather"], bind="legend")
    return (
        alt.Chart(df)
        .mark_circle(size=40)
        .encode(
            ...
            y=alt.Y("temp_max:Q", title="Daily Max Temp (°C)"),
            color=alt.Color("weather:N", title="Weather Type"),
            opacity=alt.condition(legend_select, alt.value(0.7), alt.value(0.05)),
            tooltip=[
                alt.Tooltip("date:T"),
                alt.Tooltip("weather:N", title="Weather"),
